...
import VueCodeGenerator, {
  ...
  IUseRefOptions,
  IUseStateOptions
} from '@/service/code-generator/vue';
import IPageSchema from '@/types/page.schema';
import * as dsl from '@/mock/tab-case-vue.json';
import TypeScriptCodeGenerator from '@/service/code-generator/typescript';
import { ImportType } from '@/types';
import { generateVue3File } from '@/service/code-generator';
import * as path from 'path';
import { existsSync, mkdirSync } from 'fs';

describe('vue', () => {
  const react = new VueCodeGenerator(dsl as unknown as IPageSchema, new TypeScriptCodeGenerator());
  ...
  test('should return template sentences array', () => {
      ...
      children: [
        {
          ...
          propsStrArr: [`:title="buttonTitle"`, '@click="handleClicking"']
        },
        {
    ...
    expect(react.generateTSX(tsx)).toStrictEqual([
      '<div >',
      '<Button :title="buttonTitle" @click="handleClicking" />',
  ...
  test('test use ref', () => {
    ...
    expect(react.generateUseRef(opt)).toBe("const testRef = ref<string>('1');");
  });
});
...
describe('code generation cases', () => {
  ...
  test('write file', async () => {
    ...
    generateVue3File(generationDir).then(() => {
      console.log('数据写入完成');
    }).catch(err => {
      console.log(err);
    });
